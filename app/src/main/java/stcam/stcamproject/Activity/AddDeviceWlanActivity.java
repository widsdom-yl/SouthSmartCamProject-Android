...
public class AddDeviceWlanActivity extends BaseAppCompatActivity implements BaseAdapter.OnItemClickListener
{
      ...
      switch (msg.what)
      {
        case TMsg.Msg_SearchOver:
          ...
          if (lists.size() > 0)
          {
            ...
            for (SearchDevModel searchModel : lists)
            {
              boolean exist = false;
              for (DevModel devModel : MainDevListFragment.mDevices)
              {
                if (devModel.SN.equals(searchModel.getSN()))
                {
                  exist = true;
                  break;
                }
  ...
  public void onItemClick(View view, int position)
  {
    ...
    final SearchDevModel model = mlists.get(position);
    ...
    if (dbModel == null)
    {
      editText.setText(Config.DEFAULT_DEV_PASSWORD);
    }
    else
    {
  ...
  Observer<List<SSIDModel>> observer_SSIDList = new Observer<List<SSIDModel>>()
  {
    ...
    {
      lod.dismiss();
      SearchDevModel model = mlists.get(selectPosition);
      addDevice(model);
