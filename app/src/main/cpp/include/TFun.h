...
extern "C"
{
  ...
  EXPORT char* ShareMemOpenFile(const char* FileName, HANDLE* FileHandle, HANDLE* MapHandle, u32 dwFileSize);//返回MAP Buf地址
...
#define closesocket close
  ...
  EXPORT char* FileExtName(char* FileName); //取得文件扩展名//'.txt'
  ...
  EXPORT bool InArray(i32 Value, i32 Count, ...);//数值是否在...之中
  EXPORT i32 RandomNum(i32 seed);

  //时间函数
  EXPORT i32 timeval_dec(struct timeval* tv2, struct timeval* tv1);//ms
  EXPORT i32 GetTime();
  EXPORT i64 getutime();//取得微秒级时间 tv.tv_sec*1000000 + tv.tv_usec
...
//-----------------------------------------------------------------------------
  ...
  EXPORT bool IsConnectWLAN();//外网是否已连接
  EXPORT void printPI();//打印圆周率

  EXPORT bool RectIsIntersect(TRect* r1, TRect* r2);//区域是否包含
