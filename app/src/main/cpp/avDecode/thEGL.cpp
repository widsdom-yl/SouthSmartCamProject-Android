...
#define GET_STR(x) #x
const char *vertexShaderString = GET_STR(attribute
                                           vec4 aPosition;
                                           attribute
                                           vec2 aTexCoord;
                                           varying
...
const char *fragmentShaderString = GET_STR(precision
                                             mediump float;
                                             varying
                                             vec2 vTexCoord;
                                             uniform
...
int requestEGLRender(AVFrame *Frame422)
{
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, Frame422->linesize[0], Frame422->height, 0, GL_LUMINANCE,
               GL_UNSIGNED_BYTE, Frame422->data[0]);

  glActiveTexture(GL_TEXTURE1);
  glBindTexture(GL_TEXTURE_2D, uTextureId);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, Frame422->linesize[1], Frame422->height / 2, 0, GL_LUMINANCE,
               GL_UNSIGNED_BYTE, Frame422->data[1]);

  glActiveTexture(GL_TEXTURE2);
  glBindTexture(GL_TEXTURE_2D, vTextureId);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, Frame422->linesize[2], Frame422->height / 2, 0, GL_LUMINANCE,
               GL_UNSIGNED_BYTE, Frame422->data[2]);
...
//        EGL_SURFACE_TYPE, EGL_WINDOW_BIT, EGL_NONE };
  ...
  float textureVertexData[8] = {1.0f, 0.0f,//右下
                                0.0f, 0.0f,//左下
                                1.0f, 1.0f,//右上
                                0.0f, 1.0f//左上
  };
