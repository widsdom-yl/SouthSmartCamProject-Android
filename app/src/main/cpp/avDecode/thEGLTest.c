//
// Created by guyunlong on 8/19/18.
//
...
void videoPlay(JNIEnv *env, jobject instance, jstring path_,
                 jobject surface) {

    LOGE("videoPlay ,%s(%d) \n", __FUNCTION__, __LINE__);
    const char *path = (char *) (*env)->GetStringUTFChars(env, path_, NULL);
 ...
 * **/
    ...
    GLuint programId = createProgram(vertexShaderString1,fragmentShaderString1 );
 ...
 * **/
    ...
    while (av_read_frame(fmt_ctx, pkt) >=0){
        if (pkt->stream_index == video_stream_index) {
            ret = avcodec_decode_video2(codec_ctx, yuvFrame, &got_picture,pkt);
            ...
            if (got_picture) {
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, yuvFrame->linesize[1],
                             yuvFrame->height / 2, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE,
                             yuvFrame->data[1]);
